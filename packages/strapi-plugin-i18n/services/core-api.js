...
const { has, prop, pick, omit, pipe } = require('lodash/fp');
...
const createLocalizationHandler = contentType => {
  ...
  const createFromBaseEntry = async (ctx, { entry }) => {
    ...
    const usedLocales = [entry.locale, ...entry.localizations.map(prop('locale'))];
    if (usedLocales.includes(data.locale)) {
      return ctx.badRequest('locale.already.used');
    }

    // todo: verify locale is installed => waiting for policy PR

    const sanitizedData = {
      ...copyNonLocalizedAttributes(contentType, entry),
      ...sanitizeInput(data),
      locale: data.locale,
      localizations: [entry.id, ...entry.localizations.map(prop('id'))],
    };
  ...
  if (isSingleType(contentType)) {
    return async function(ctx) {
      const entry = await strapi.entityService.find({}, { model: contentType.uid });
      if (!entry) {
